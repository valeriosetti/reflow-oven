...
// 100 logarithmically spaced brightness levels between 0x0000 and 0xFFFF
// Each of them is computed as --> 2^(x*a/b)-1
// where:
//		- a: is the size of the PWM register (which defines also the maximum value for the PWM)
// 		- b: is the number of desired levels
//		- x: is the current level
// In our case it's --> 2^(x*16/100)-1
uint16_t brightness_levels[101] = {	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0001, 0x0001, 0x0001,
									0x0002, 0x0002, 0x0002, 0x0003, 0x0003, 0x0004, 0x0004, 0x0005, 0x0006, 0x0007,
									0x0008, 0x0009, 0x000A, 0x000B, 0x000D, 0x000F, 0x0010, 0x0012, 0x0015, 0x0017,
									0x001A, 0x001E, 0x0021, 0x0025, 0x002A, 0x002F, 0x0035, 0x003B, 0x0042, 0x004A,
									0x0053, 0x005D, 0x0068, 0x0074, 0x0082, 0x0092, 0x00A3, 0x00B6, 0x00CC, 0x00E4,
									0x00FF, 0x011D, 0x013E, 0x0164, 0x018D, 0x01BC, 0x01F0, 0x022B, 0x026C, 0x02B5,
									0x0307, 0x0362, 0x03C7, 0x0439, 0x04B8, 0x0546, 0x05E4, 0x0695, 0x075B, 0x0838,
									0x092F, 0x0A43, 0x0B77, 0x0CD0, 0x0E51, 0x0FFF, 0x11DF, 0x13F8, 0x164F, 0x18ED,
									0x1BDA, 0x1F1E, 0x22C5, 0x26D9, 0x2B68, 0x307F, 0x3630, 0x3C8B, 0x43A5, 0x4B94,
									0x5471, 0x5E59, 0x696A, 0x75C7, 0x8398, 0x9307, 0xA446, 0xB78A, 0xCD11, 0xE51F,
									0xFFFF };
...
void PCD8544_InitIO(void) {
	...
	// 		CLK
	GPIO_InitStruct.Pin = PCD8544_CLK_PIN;
	HAL_GPIO_Init(PCD8544_CLK_PORT, &GPIO_InitStruct);
	// 		DIN
	GPIO_InitStruct.Pin = PCD8544_DIN_PIN;
	...
	__TIM4_CLK_ENABLE();
	hTIM.Instance = TIM4;
	hTIM.Init.Prescaler = 0x2;
	hTIM.Init.CounterMode = TIM_COUNTERMODE_UP;
	hTIM.Init.Period = 0xFFFF;
	hTIM.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
	hTIM.Init.RepetitionCounter = 0;
	HAL_TIM_PWM_Init(&hTIM);
	// Configure OC for PWM generation
	TIM3_OC_init.OCMode = TIM_OCMODE_PWM1;
	TIM3_OC_init.Pulse = 0x0000;	// set the PWM to the min value at the beginning
	TIM3_OC_init.OCPolarity = TIM_OCPOLARITY_HIGH;
	TIM3_OC_init.OCFastMode = TIM_OCFAST_DISABLE;
	TIM3_OC_init.OCIdleState = TIM_OCIDLESTATE_RESET;
	HAL_TIM_PWM_ConfigChannel(&hTIM, &TIM3_OC_init, TIM_CHANNEL_1);
	// Enable the PWM
...
void PCD8544_SetBacklight(uint8_t value)
{
	// Threshold the input value
	if (value>100)
		value = 100;
	// Set the desired value to the PWM register
	__HAL_TIM_SetCompare(&hTIM, TIM_CHANNEL_1, (uint16_t)brightness_levels[value]);
}
